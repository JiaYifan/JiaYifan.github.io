...
转自[重新介绍 JavaScript（JS 教程）](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/A_re-introduction_to_JavaScript)
...
!> JavaScript 采用“IEEE 754 标准定义的**双精度64位格式**”（"double-precision 64-bit format IEEE 754 values"）表示数字

!> JavaScript 不区分整数值和浮点数值，所有数字在 JavaScript 中均用浮点数值表示

!>  JavaScript 中语句块（blocks）是没有作用域的，**只有函数有作用域**。

!> 简单的说，`==`会类型自适应，也就是自动类型转换；`===`是严格相等的比较。

!> `&&` 和 `||` 运算符使用短路逻辑（short-circuit logic），前者可以测空，后者可以设默认值

!> `Array.length` 并不总是等于数组中元素的个数，这个属性的值通常比数组最大索引大 1。（数组中间可以断档）

!> forEach、for-in与for-of的区别 [>>>>](#for)

!> IIFES立即执行函数表达式内的变量是局部可见的，在外面就访问不了，其中的函数也是如此，这允许引擎去做更多的优化？

!> 当使用在函数中时，`this` 指代当前的对象，**也就是调用了函数的对象**。

!> 把方法定义在构造函数的prototype上，好处是子子孙孙都可以动态继承

!> ...args剩余参数语法允许我们将一个不定数量的参数表示为一个数组。

!> 每当执行一个函数时，都会创建一个作用域对象（scope object），用来保存在这个函数中创建的局部变量。它和被传入函数的变量一起被初始化。与全局对象（在浏览器里面是当做 window 对象来访问的）不同的是：第一，每次函数被执行的时候，就会创建一个新的，特定的作用域对象；第二，你不能从 JavaScript 代码中直接访问作用域对象，也没有可以遍历当前的作用域对象里面属性的方法。
...
# 引言
为什么会有这一篇“重新介绍”呢？因为 JavaScript 堪称[世界上被人误解最深的编程语言](http://javascript.crockford.com/javascript.html)。虽然常被嘲为“玩具语言”，但在它看似简洁的外衣下，还隐藏着强大的语言特性。 JavaScript 目前广泛应用于众多知名应用中，对于网页和移动开发者来说，深入理解 JavaScript 就尤有必要。
...
几个月后，Microsoft 随着 IE 3 推出了一个与之基本兼容的语言 JScript。又几个月后，Netscape 将 JavaScript 提交至 [Ecma International](http://www.ecma-international.org/)（一个欧洲标准化组织）， ECMAScript 标准第一版便在 1997 年诞生了，随后在 1999 年以 [ECMAScript 第三版](http://www.ecma-international.org/publications/standards/Ecma-262.htm)的形式进行了更新，从那之后这个标准没有发生过大的改动。由于委员会在语言特性的讨论上发生分歧，ECMAScript 第四版尚未推出便被废除，但随后于 2009 年 12 月发布的 ECMAScript 第五版引入了第四版草案加入的许多特性。第六版标准已经于2015年六月发布。
...
…哦，还有看上去有些…奇怪的 [undefined](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/undefined)（未定义）类型和 [null](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/null)（空）类型。此外还有[Array](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Array)（数组）类型，以及分别用于表示日期和正则表达式的 [Date](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Date)（日期）和 [RegExp](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/RegExp)（正则表达式），这三种类型都是特殊的对象。严格意义上说，[Function](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Function)（函数）也是一种特殊的对象。所以准确来说，JavaScript 中的类型应该包括这些：
...
# 数字
根据语言规范，JavaScript 采用“IEEE 754 标准定义的**双精度64位格式**”（"double-precision 64-bit format IEEE 754 values"）表示数字。据此我们能得到一个有趣的结论，和其他编程语言（如 C 和 Java）不同，JavaScript 不区分整数值和浮点数值，所有数字在 JavaScript 中均用浮点数值表示，所以在进行数字运算的时候要特别注意。看看下面的例子：
...
parseInt("11", 2); // 3
```
JavaScript 还有一个类似的内置函数 [parseFloat()](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/parseFloat)，用以解析浮点数字符串，与`parseInt()`不同的地方是，`parseFloat()`只应用于解析十进制数字。
...
myVarVariable // myVarVariable *is* visible out here
myLetVariable // myLetVariable is *not* visible out here
myConstVariable // myConstVariable is *not* visible out here
```
...
```
> 插一个`forEach`、`for-in`与`for-of`的区别，[原文](https://www.cnblogs.com/xueshanshan/p/8487438.html)
<span id="for"></span>
* `foreach` 方法没办法使用 `break` 语句跳出循环，或者使用`return`从函数体内返回
* `for-in`建议只用在以字符串的值为key的对象使用
```javascript
...
```
  ...
  * 循环不仅会遍历数组元素，还会遍历任意其他自定义添加的属性，如，`objArr`上面包含自定义属性，`objArr.name`，那这次循环中也会出现`此name`属性
  * 某些情况下，上述代码会以随机顺序循环数组
* `for-of`
  * 不同于 `forEach()`，可以使用 `break`, `continue` 和 `return`
  * 不仅仅支持数组的遍历。同样适用于很多类似数组的对象，因此也支持字符串的遍历
...
```
> `prompt()`方法用于显示可提示用户进行输入的对话框。点击按钮进行尝试

<input type="button" value="请输入您的名字" onclick="dispPrompt()" />
...
```
遍历数组的另一种方法是使用 `for...in` 循环。注意，如果有人向 `Array.prototype` 添加了新的属性，使用这样的循环这些属性也同样会被遍历。所以并不推荐使用这种方法遍历数组：
```javascript
for (var i in a) {
  // Do something with a[i]
}
...
> toLocaleString和toString的区别，主要是
```javascript
var dt = new Date();
...
```
我们new一个`Person`实例再打印出来看看
```javascript
Person {first: "Simon", last: "Willison", fullName: ƒ, fullNameReversed: ƒ}
    first: "Simon"
    fullName: ƒ personFullName()
    fullNameReversed: ƒ personFullNameReversed()
    last: "Willison"
    __proto__: Object
```
...
Person {first: "Simon", last: "Willison"}
    ...
    fullName: ƒ ()
    fullNameReversed: ƒ ()
    constructor: ƒ Person(first, last)
    __proto__: Object
```
...
所以当调用 `makeAdder` 时，解释器创建了一个**作用域对象**，它带有一个属性：`a`，这个属性被当作参数传入 `makeAdder` 函数。然后 `makeAdder` 返回一个新创建的函数。通常 JavaScript 的垃圾回收器会在这时回收 `makeAdder` 创建的作用域对象，但是返回的函数却保留一个**指向那个作用域对象的引用**。结果是这个作用域对象不会被垃圾回收器回收，直到指向 `makeAdder` 返回的那个函数对象的引用计数为零。
